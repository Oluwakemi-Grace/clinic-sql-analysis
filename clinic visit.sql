...
SELECT description, COUNT(description) AS diagnosis_count
FROM medication
GROUP BY description
ORDER BY diagnosis_count DESC;

SELECT appointment_date, COUNT(appointment_date) AS visit_peak
FROM appointment
GROUP BY appointment_date 
ORDER BY appointment_date;

SELECT dr.name, dr.doctor_id, dr.speciality_id, COUNT(ap.doctor_id) AS dr_productivity
FROM appointment ap
JOIN doctor dr ON ap.doctor_id = dr.doctor_id
GROUP BY dr.name, dr.doctor_id, dr.speciality_id
ORDER BY dr_productivity DESC;

SELECT name, patient_id,
  2025 - YEAR(dob) AS age
FROM patient
ORDER BY age DESC;

SELECT pt.name, pt.dob, pt.patient_id, pt.gender,apt.appointment_id, md.medication_id,md.name, md.description, apt.notes
FROM appointment apt
LEFT JOIN prescription prp ON apt.appointment_id = prp.appointment_id
LEFT JOIN medication md ON md.medication_id = prp.medication_id
LEFT JOIN patient pt ON pt.patient_id = apt.patient_id
GROUP BY pt.name, pt.dob, pt.patient_id, pt.gender,apt.appointment_id, md.medication_id,md.name, md.description, apt.notes
ORDER BY pt.dob;
...
Error Code: 1054. Unknown column 'apt.note' in 'field list'
